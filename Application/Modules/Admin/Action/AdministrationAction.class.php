...
**
**/
class AdministrationAction extends AuthAction{
	...
	public function index(){
		...
		if(isset($_GET['order_goods'])){
			$order=$_GET['order_goods']." desc";
		}
...
//订单状态：0(已取消)10(默认):未付款;20:已付款;30:已发货;40:已收货;
	...
	public function administration(){
		if($_POST){
			...
			$where["order_sn"]=$data["pay_sn"];
			$data_info=M("g_order_info")->where($where)->find();
			$data_log=D("g_pay_log")->where($menuwhere)->select();
	...
	public function order_exchange(){
		if($_POST){
			...
			if($order_res!==false && $shop_res!==false){
                ...
                ///用户返利 返积分 认证等
                $this->set_member_e xchange($order_info);///设置用户兑换
                $model_order->commit();
                $shop_model->commit();
...
public function set_member_exchange($order_info){
	  ...
	  $member_tuijian_data=array();//返利记录
	        ...
	        //增加用户积分 1：1
	        $integral=round($data['order_amount']);
             if($integral>0){
	        	$log=array();
	        	$log['order_id']=$order_info['order_id'];
	        ...
	        //未认证用户 消费大于 188
	        if($data['order_amount']>=188 && empty($member_info['member_status'])){
	        	//认证
