(function($){
    $.fn.yxMobileSlider = function(settings){
        ...
        return this.each(function(){
            ...
            /*$("img", oLi).css('cssText','max-width:100%;max-height:100%;');*/
            //初始化焦点容器及按钮
            ...
            $(window).bind('resize load', function(){
                ...
                //oLi.width(this_width).height(_this.height());//设定滚动单元宽高  //Update hkj
                oMover.width(num * oLi.width());
               
                oFocusContainer.width(_this.width()).height(_this.height() * 0.15).css({  
                 //Update hkj
                // oFocusContainer.width(this_width).height(_this.height() * 0.15).css({   
                    zIndex: 2
                });//设定焦点容器宽高样式
                _this.fadeIn(300);
            });
            ...
            function mobileSettings(){
                //Update hkj 最大宽度630 最小宽度310 
                var w_weidth=$(window).width();
                if (w_weidth>630) {
                    w_weidth=630;
                }else if (w_weidth<320) {
                    w_weidth=310;
                }else{
                    w_weidth=w_weidth-10;
                }
                moveWidth =w_weidth ;
                var iScale = w_weidth / s.width;
                _this.height(s.height * iScale).width(w_weidth);
                oMover.css({
