...
class MallpaymentAction extends UserAction {
       ...
       //die;
        ...
        if(!empty($result['error'])) {
         ...
         if($payment_code=='wxpay'){
            //第三方API支付
            $this->_wx_api_pay($result['order_pay_info'], $result['payment_info']);
        }else{
             //第三方API支付
...
*/
    ...
    if (!isset($_GET['code']))
    {
        ...
        $url=U('Wap/Mallpayment/index',array('pay_sn'=>$order_pay_info['pay_sn']));
    ...
    $unifiedOrder->setParameter("total_fee",$order_pay_info['pay_amount']);//总金额
...
require_once './api/payment/wpay/WxPay.JsApiPay.php';
//require_once  './api/payment/'.$payment_info['payment_code'].'/log.php';

//初始化日志
//$logHandler= new CLogFileHandler("./api/payment/wpay/lib/logs/".date('Y-m-d').'.log');
...
$url=U('Wap/Returnpayment/index');
$url="http://".$_SERVER['HTTP_HOST'].$url;
$input->SetNotify_url("notify_url",$url);//通知地址 
//$input->SetNotify_url("http://".$_SERVER['HTTP_HOST']."/api/payment/wxpay/notify.php");
$input->SetTrade_type("JSAPI");
$input->SetOpenid($openId);
$order = WxPayApi::unifiedOrder($input);
...
//获取共享收货地址js函数参数
//$this->editAddress = $tools->GetEditAddressParameters();
$sub=$this->submit($jsApiParameters,$order_pay_info);
    echo $sub;
        exit;
    }

public function submit($jsApiParameters,$order_pay_info){
    $html='<html>
<head>
