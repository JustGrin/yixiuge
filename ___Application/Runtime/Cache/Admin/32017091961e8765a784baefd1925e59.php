						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									...
									<span class="mainDescription">&nbsp;&nbsp;员工</span>
								</div>
								<nav id="cl-effect-10" class="links cl-effect-10">
									<a data-hover="新增" href="<?php echo U('admin/Personnel/members_edit');?>"><span>新增</span></a>
								</nav>
								<!-- <ol class="breadcrumb">
									<li>
										<span>Pages</span>
		...
		<div class="panel-body">
				...
				<div class="form-group">
					  ...
					  <label for="account" class="sr-only">
						...
						<input type="text" name="name" value="<?php echo ($_GET["name"]); ?>" id="account" placeholder="姓名" class="form-control">
				 ...
				 <div class="form-group">
					...
					<select class="form-control" id="type" name="p_id">
						<option value="0" >全部部门</option>
					    <?php if(is_array($p_list)): $i = 0; $__LIST__ = $p_list;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($vo["id"]); ?>" <?php if($_GET['p_id'] == $vo['id']): ?>selected<?php endif; ?> ><?php echo ($vo["p_name"]); ?></option>
							<?php if(is_array($vo['tree'])): $i = 0; $__LIST__ = $vo['tree'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$voo): $mod = ($i % 2 );++$i;?><option value="<?php echo ($voo["id"]); ?>" <?php if($_GET['p_id'] == $voo['id']): ?>selected<?php endif; ?> >┗━<?php echo ($voo["p_name"]); ?></option><?php endforeach; endif; else: echo "" ;endif; endforeach; endif; else: echo "" ;endif; ?>
					</select>
				</div>
				...
				<div class="form-group">
					...
					<select class="form-control" id="status" name="status">
						...
						<option value="1" <?php if($_GET['status'] ==1): ?>selected<?php endif; ?>>有效</option>
						<option value="0" <?php if($_GET['status'] ==='0'): ?>selected<?php endif; ?>>无效</option>
					</select>
				</div>	
...
<table class="table table-hover" id="sample-table-1">
      ...
      <?php if(is_array($list['list'])): $key = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
						<td class="center"><?php echo ($vo["name"]); ?><i/></td>
						<td> <?php echo (($vo["p_name"])?($vo["p_name"]):'-'); ?></td>
						<td class="hidden-xs"><?php echo (($vo["admin_name"])?($vo["admin_name"]):'-'); ?>(帐号：<?php echo (($vo["account"])?($vo["account"]):''); ?>)</td>
						<td><?php echo ($vo['status']?'有效':'无效'); ?></td>
						<td class="center">
						<div class="visible-md visible-lg hidden-sm hidden-xs">

							<a href="<?php echo U("Admin/Personnel/members_edit",array('id'=>$vo['id']));?>" class="btn btn-transparent btn-xs" tooltip-placement="top" data-toggle="tooltip" data-original-title="编辑"><i class="fa fa-pencil"></i></a>
							
							<a href="<?php echo U("Admin/Personnel/members_del",array('id'=>$vo['id']));?>" class="btn btn-transparent btn-xs tooltips" tooltip-placement="top" data-toggle="tooltip" data-original-title="删除" onclick="return confirm('确定删除该数据？')"><i class="fa fa-times fa fa-white"></i></a>
