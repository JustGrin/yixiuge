					...
					<div class="wrap-content container" id="container">
						...
						<section id="page-title">
							<div class="row">
								...
								<a data-hover="新增" href="<?php echo U('admin/goods/goods_edit');?>"><span>新增</span></a>
...
<table class="table table-hover" id="sample-table-1">
        ...
        <?php if(is_array($list['list'])): $key = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr  >
						...
						<td id="show<?php echo ($vo["goods_id"]); ?>">
							<?php if($vo["is_show_index"] == 1): ?><a href="javascript:void(0)"
							   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
							...
							<a href="javascript:void(0)"
							   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
							   data-toggle="tooltip" onclick="changeShow(<?php echo ($vo["goods_id"]); ?>,<?php echo ($vo["is_show_index"]); ?>)"
							   data-original-title="显示首页"><i class="fa fa-check-circle" style="color:#ccc;"  value="1"></i></a><?php endif; ?>
						</td>
						
						<td id="sell<?php echo ($vo["goods_id"]); ?>">
							<?php if($vo["is_sell_out"] == 1): ?><a href="javascript:void(0)"
							   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
							   data-toggle="tooltip" onclick="changeSell(<?php echo ($vo["goods_id"]); ?>,<?php echo ($vo["is_sell_out"]); ?>)"
							   data-original-title="售罄"><i class="fa fa-check-circle" ></i></a>
							<?php else: ?>
							<a href="javascript:void(0)"
							   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
							   data-toggle="tooltip" onclick="changeSell(<?php echo ($vo["goods_id"]); ?>,<?php echo ($vo["is_sell_out"]); ?>)"
							   data-original-title="未售罄"><i class="fa fa-check-circle" style="color:#ccc;"  value="1"></i></a><?php endif; ?>
						</td>
						...
						<div class="visible-md visible-lg hidden-sm hidden-xs">
                            ...
                            <a href="<?php echo U("Admin/Goods/Goods_edit",array('id'=>$vo['goods_id']));?>"
							class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
							data-toggle="tooltip" data-original-title="编辑">
							<i class="fa fa-pencil"></i></a>

                            <a href="<?php echo U("Admin/Goods/Goods_del",array('id'=>$vo['goods_id']));?>"
							class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
...
<script type="text/javascript">
	function changeStatus(id,value){
		...
		if(confirm('确定'+sale+'?')){
			$.ajax({
				url:'<?php echo U("Admin/goods/is_on_sale");?>',
				type:'get',
	...
	function changeShow(id,value){
		...
		if(confirm('确定'+sale+'?')){
			$.ajax({
				url:'<?php echo U("Admin/goods/is_show_index");?>',
				type:'get',
				data:'id='+id+'&value='+value,
				dataType:'json',
				success:function(data){
	...
	function changeSell(id,value){
		...
		if(confirm('确定'+sale+'?')){
			$.ajax({
				url:'<?php echo U("Admin/goods/is_sell_out");?>',
				type:'get',
				data:'id='+id+'&value='+value,
				dataType:'json',
				success:function(data){
					if(data.status==1){
						$('#sell'+id).empty();
						if(value){
							var content = '<a href="javascript:void(0)" class="btn btn-transparent btn-xs" tooltip = "Edit" tooltip - placement = "top" data - toggle = "tooltip" onclick = "changeSell('+id+','+value+')" data - original - title = "'+sale+'" > <i class="fa fa-check-circle" ></i></a>'
						}else{
							var content = '<a href="javascript:void(0)" class="btn btn-transparent btn-xs" tooltip = "Edit" tooltip - placement = "top" data - toggle = "tooltip" onclick = "changeSell('+id+','+value+')" data - original - title = "'+sale+'" > <i class="fa fa-check-circle" style="color:#ccc;"  value="1"></i></a>'
						}
						$('#sell'+id).append(content);
						layer.msg(sale+'确认成功',{icon:6});
					}else{
						layer.msg(data.error,{icon:5});
					}
				}
