		...
		<div class="panel-body">
				...
				<div class="form-group">
					  <label for="member_name" class="sr-only">
						...
						<input type="text" name="member_name" value="<?php echo ($_GET["member_name"]); ?>" id="member_name" placeholder="姓名" class="form-control">
				</div>
				<div class="form-group">
					  <label for="mobile" class="sr-only">
						电话
					  </label>
						<input type="text" name="mobile" value="<?php echo ($_GET["mobile"]); ?>" id="mobile" placeholder="电话" class="form-control">
				</div>
				<div class="form-group">
					<label for="is_check" class="sr-only">
						冻结
					</label>
					<select class="form-control" id="is_check" name="is_check">
						<option value="all" <?php if($_GET['is_check'] =='all'): ?>selected<?php endif; ?>>全部状态</option>
						<option value="0" <?php if($_GET['is_check'] =='0'): ?>selected<?php endif; ?>>未审核</option>
						<option value="1" <?php if($_GET['is_check'] ==1): ?>selected<?php endif; ?>>通过</option>
						<option value="2" <?php if($_GET['is_check'] ==2): ?>selected<?php endif; ?>>不通过</option>
					</select>
				</div>
		</div>
 <div class="panel-body">
	...
	<div class="form-group">
	  ...
	  <p class="input-group input-append datepicker date">
			<input type="text" class="form-control" name="start_time" value="<?php echo ($_GET['start_time']); ?>" placeholder="注册时间"/>
		...
		--
		 <p class="input-group input-append datepicker date">
			<input type="text" class="form-control" name="end_time" value="<?php echo ($_GET['end_time']); ?>" placeholder="注册时间"/>
			<span class="input-group-btn">
				<button type="button" class="btn btn-default">
					<i class="glyphicon glyphicon-calendar"></i>
				</button> </span>
...
<table class="table table-hover" id="sample-table-1">
      ...
      <?php if(is_array($list['list'])): $key = 0; $__LIST__ = $list['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr  >
						...
						<td> 
							<a href="<?php echo U('Admin/Member/member_edit',array('id'=>$vo['id']));?>" >
							<?php echo ($vo["member_card"]); ?>
						</a>
						</td>
						<td><?php echo ($vo["member_name"]); ?></td>

						<td><?php echo (date("Y-m-d H:i",$vo["change_time"])); ?>	</td>
						<td id="check<?php echo ($vo["id"]); ?>">
							<select class="is_check" data-id="<?php echo ($vo["id"]); ?>" name="is_check" >
								<option value="0" <?php if($vo["is_check"] == 0): ?>selected<?php endif; ?>>未审核</option>
								<option value="1" <?php if($vo["is_check"] == 1): ?>selected<?php endif; ?>>通过</option>
								<option value="2" <?php if($vo["is_check"] == 2): ?>selected<?php endif; ?>>不通过</option>
							</select>
						</td>
						<td>
							<textarea class="remark" data-id="<?php echo ($vo["id"]); ?>" name= 'remark' ><?php echo ($vo["remark"]); ?></textarea>
						</td>
						<td class="center">
						<div class="visible-md visible-lg hidden-sm hidden-xs">
							<a href="<?php echo U('Wap/carinfo/msg_detail',array('id'=>$vo['id']));?>" class="btn btn-transparent btn-xs" target="view_window" tooltip="Edit"  tooltip-placement="top" data-toggle="tooltip" data-original-title="查看"><i class="fa fa-file-text-o"></i></a>
						</div>
						</td>
...
<script type="text/javascript">
	...
	function changeCheck(id,value,type){
		...
		if(confirm('确定设置?')){
			$.ajax({
				url:'<?php echo U("Admin/Usedcar/do_msg_check");?>',
				type:'get',
