						...
						<section id="page-title">
							<div class="row">
								<div class="col-sm-8">
									...
									<span class="mainDescription">&nbsp;&nbsp;配送管理</span>
								</div>
								<nav id="cl-effect-10" class="links cl-effect-10">
									<a data-hover="新增" href="<?php echo U('admin/goods/goods_shipping_addalter');?>"><span>新增</span></a>
								</nav>
							</div>
						</section>
						<!-- end: PAGE TITLE -->
	...
	<!-- 筛选查询 -->
			<form role="form" class="form-inline">
				<div class="panel-body">
						<div class="form-group">
								<input type="text" name="shipping_name" id="shipping_name" value="<?php echo ($_GET["shipping_name"]); ?>" placeholder="名称" class="form-control">
						</div>
						<div class="form-group">
							<input type="text" placeholder="代号" name="shipping_code" id="shipping_code" value="<?php echo ($_GET["shipping_code"]); ?>" class="form-control">
						</div>
						&nbsp;
...
<table class="table table-hover" id="sample-table-1">
				...
				<thead>
					<tr>
						...
						<th class="center">是否用于发货</th>
						<!--<th class="hidden-xs">配送方式描述</th>-->
						<!--<th>保价费</th>-->
						<!--<th>货到付款</th>-->
						<th class="center">配送方式是否禁用</th>
      ...
      <?php if(is_array($data['list'])): $key = 0; $__LIST__ = $data['list'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($key % 2 );++$key;?><tr>
						<td class="center">#</td>
						<td class="center"><?php echo ($vo["shipping_name"]); ?></td>
						<td class="center"> <?php echo ($vo["shipping_code"]); ?></td>
						<td id="deliver<?php echo ($vo["shipping_id"]); ?>">
							<?php if($vo["is_deliver"] == 1): ?><a href="javascript:void(0)"
								   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
								   data-toggle="tooltip" onclick="changedeliver(<?php echo ($vo["shipping_id"]); ?>,<?php echo ($vo["is_deliver"]); ?>)"
								   data-original-title="取消使用"><i class="fa fa-check-circle" ></i></a>
								<?php else: ?>
								<a href="javascript:void(0)"
								   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
								   data-toggle="tooltip" onclick="changedeliver(<?php echo ($vo["shipping_id"]); ?>,<?php echo ($vo["is_deliver"]); ?>)"
								   data-original-title="使用"><i class="fa fa-check-circle" style="color:#ccc;"  value="1"></i></a><?php endif; ?>
						</td>
						<!--<td><?php echo ($vo["shipping_desc"]); ?></td>-->
						<!--<td><?php echo ($vo["insure"]); ?></td>-->
						<!--<td><?php if($vo["support_cod"] == 1): ?>是<?php else: ?>否<?php endif; ?></td>-->
						<td class="center"><?php echo ($vo['enabled']?'否':'是'); ?></td>
						<td class="center">
									<a href="<?php echo U('Admin/Goods/goods_shipping_addalter',array('shipping_id'=>$vo['shipping_id']));?>"
									   class="btn btn-transparent btn-xs"  tooltip="Edit"  tooltip-placement="top"
									   data-toggle="tooltip" data-original-title="编辑"><i class="fa fa-pencil"></i></a>

									<a href="<?php echo U('Admin/Goods/goods_shipping_del',array('shipping_id'=>$vo['shipping_id']));?>"
									   class="btn btn-transparent btn-xs tooltips" tooltip-placement="top"
									   data-toggle="tooltip" data-original-title="删除"
									   onclick="return confirm('确定删除该数据？')"><i class="fa fa-times fa fa-white"></i></a>
						</td>
					</tr><?php endforeach; endif; else: echo "" ;endif; ?>
				</tbody>
			</table>
					<div class="row">
						<div class="col-md-12 pages" align="center"><?php echo ($data["page"]); ?></div>
					</div>
					<?php if(empty($data['page'])): ?>暂无数据<?php endif; ?>
...
<script>
	function changedeliver(id,value){
		if(value==0){
			value =1;
			var deliver='使用';
		}else{
		...
		if(confirm('确定'+deliver+'?')){
			$.ajax({
				url:'<?php echo U("Admin/goods/deliver_able");?>',
				type:'get',
				...
				success:function(data){
					if(data.status==1){
						$('#deliver'+id).empty();
						if(value){
							var content = '<a href="javascript:void(0)" class="btn btn-transparent btn-xs" tooltip = "Edit" tooltip - placement = "top" data - toggle = "tooltip" onclick = "changedeliver('+id+','+value+')" data - original - title = "'+deliver+'" > <i class="fa fa-check-circle" ></i></a>'
						}else{
							var content = '<a href="javascript:void(0)" class="btn btn-transparent btn-xs" tooltip = "Edit" tooltip - placement = "top" data - toggle = "tooltip" onclick = "changedeliver('+id+','+value+')" data - original - title = "'+deliver+'" > <i class="fa fa-check-circle" style="color:#ccc;"  value="1"></i></a>'
						}
						$('#deliver'+id).append(content);
						layer.msg('已'+deliver+'该快递',{icon:6});
					}else{
