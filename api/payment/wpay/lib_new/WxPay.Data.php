...
class WxPayDataBase
{
	...
	**/
     ...
     * @throws WxPayException
     */
	public function JsPayFromXml($xml)
	{	
		
		$res_xml=simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA);
		 if(empty($res_xml)){
			return false;
		 }
		 $encode=json_encode($res_xml);
		$res =@json_decode($encode,true);
		 $this->values = $res;
		return $res;
        //将XML转为array 
      // $this->values = json_decode(json_encode(simplexml_load_string($xml, 'SimpleXMLElement', LIBXML_NOCDATA)), true);		
...
class WxPayResults extends WxPayDataBase
{
	 ...
	 * 检测签名
	 */
	public function JsPayCheckSign()
	{
		if(!$this->IsSignSet()){
			return true;
		}
     ...
     * @throws WxPayException
     */
	public static function JsPayInit($xml)
	{	
		 if(!$xml){
			   $data['error']="XML格式错误";
				return $data;
		...
		$obj = new self();
		$res_xml=$obj->JsPayFromXml($xml);
		
		 if(empty($res_xml)){
			   $data['error']="XML解析失败";
				return $data;
		 }
		$sign= $obj->JsPayCheckSign();
		 if(empty($sign)){
			   $data['error']="签名错误";
				return $data;
		 }
